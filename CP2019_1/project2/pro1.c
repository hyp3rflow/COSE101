...
void newBlock() {
    ...
    block.pos_x = rand()%8; // example value of position 예시값
    block.pos_y = 0; // example value of position 예시값
    block.isactive = 1;
    gameScreen[block.pos_x][block.pos_y] = block.num;
    
    block2.pos_x = block.pos_x; // example value of position 예시값
    block2.pos_y = 1; // example value of position 예시값
    block2.isactive = 1;
            ...
            switch (input_blockControl) {
                ...
                case SPACE:
                    for(int i=block.pos_x+1; i<X; i++){
                      int incnt = 0;
                      if(incnt) continue;
                      if(gameScreen[i][block.pos_y] || (i == X-1 && !gameScreen[i][block.pos_y])){
                        gameScreen[i-1][block.pos_y] = block.num;
                    ...
                    for(int i=block2.pos_x+1; i<X; i++){
                      int incnt = 0;
                      if(incnt) continue;
                      if(gameScreen[i][block2.pos_y] || (i == X-1 && !gameScreen[i][block2.pos_y])){
                        gameScreen[i-1][block2.pos_y] = block2.num;
...
void moveBlock(int direction) { // 좌,우,아래 입력시 움직임 함수    Moving blocks for left, reight, and down input
    switch (direction) {
        case LEFT:
             ...
             */
            if(block.pos_y > 0 && gameScreen[block.pos_x][block.pos_y-1] == 0 && block2.pos_y > 0 && gameScreen[block2.pos_x][block2.pos_y-1] == 0){
        ...
        case RIGHT:
             ...
             */
            if(block.pos_y < Y && gameScreen[block.pos_x][block.pos_y+1] == 0 && block2.pos_y > 0 && gameScreen[block2.pos_x][block2.pos_y+1] == 0){
              gameScreen[block.pos_x][block.pos_y+1] = block.num;
              gameScreen[block.pos_x][block.pos_y] = 0;
              block.pos_y++;
...
int checkAdjacentBlock(int x, int y) { //Merging 조건 확인 함수        Checking merging condition
  ...
  //vertical first
    if(y > 0 && y < Y){
      if(gameScreen[x][y] == '+' || gameScreen[x][y] == '-'){
        if(gameScreen[x][y-1] > 0 && gameScreen[x][y-1] < 70 && gameScreen[x][y+1] > 0 && gameScreen[x][y+1] < 10){
          if(gameScreen[x][y] == '+'){
            gameScreen[x][y-1] += gameScreen[x][y+1];
          }
          else{
            gameScreen[x][y-1] > gameScreen[x][y+1] ? gameScreen[x][y-1] -= gameScreen[x][y+1] : gameScreen[x][y-1] = gameScreen[x][y+1] - gameScreen[x][y-1];
          }
          gameScreen[x][y] = 0;
          gameScreen[x][y+1] = 0;
    ...
    if(x > 0 && x < X){
      if(gameScreen[x][y] == '+' || gameScreen[x][y] == '-'){
        if(gameScreen[x-1][y] > 0 && gameScreen[x-1][y] < 70 && gameScreen[x+1][y] > 0 && gameScreen[x+1][y] < 70){
          if(gameScreen[x][y] == '+'){
            gameScreen[x+1][y] += gameScreen[x-1][y];
          }
          else{
            gameScreen[x+1][y] > gameScreen[x-1][y] ? gameScreen[x+1][y] -= gameScreen[x-1][y] : gameScreen[x+1][y] = gameScreen[x-1][y] - gameScreen[x+1][y];
          }
          gameScreen[x][y] = 0;
          gameScreen[x-1][y] = 0;
