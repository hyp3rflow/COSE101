...
int main() {
    ...
    for (int i = 0; i < 2; i++) {
        text[i] = rand() % 301;
        voice[i] = rand() % 700 + 1;
        data[i] = (float)(rand() % 14951 + 50) / 100;
    }
    //AVERAGE value initialized
    voice[AVERAGE] = 0;
    text[AVERAGE] = 0;
    data[AVERAGE] = 0;
    
    //Average value reinitialized
    for (int i = 0; i < AVERAGE; i++) {
        voice[AVERAGE] += voice[i];
        text[AVERAGE] += text[i];
        data[AVERAGE] += data[i];
    }
    voice[AVERAGE] = roundiv(voice[AVERAGE], 3);
    text[AVERAGE] = roundiv(text[AVERAGE], 3);
    data[AVERAGE] /= 3;
...
void func3_print(int voice[], int text[], float data[]) { // 이거 안쓰는 function임
    do {
        system("cls");
        printf("================================\n");
        printf("Input your usages of voice : %d\n", voice[CURRENT]);
...
void feeCalc(int plan, int voice[], int text[], float data[], float extradata[], int fee[], int rec_fee[]) {
    ...
    if (plan == BASIC) {
        for (int i = 0; i <= CURRENT; i++) {
            ...
            if (data[i] - (i > 0 ? extradata[i - 1] : 0) > 2.00) {
                fee[i] += (data[i] - (i > 0 ? extradata[i - 1] : 0) - 2.00) * 10000;
        ...
        for (int i = 0; i <= CURRENT; i++) {
            ...
            if (data[i] - (i > 0 ? extradata[i - 1] : 0) > 30.00) {
                fee[i] += (data[i] - (i > 0 ? extradata[i - 1] : 0) - 30.00) * 5000;
                // fee[i] += (int)(10*((data[i] -(i>0 ? extradata[i-1] : 0) - 30.00))) * 500;
            }
        }
    }
    
    //RecommendFee Calculator
    rec_fee[BASIC] = 20000;
    rec_fee[MORED] = 45000;
    ...
    if (data[AVERAGE] > 2.00) {
        rec_fee[BASIC] += (roundiv((int)(1000 * data[AVERAGE]), 10) - 200.00) * 100;
        if (data[AVERAGE] > 30.00) rec_fee[MORED] += (roundiv((int)(1000 * data[AVERAGE]), 10) - 3000.00) * 50;
...
int roundiv(int a, int b){
    int tmp;
    tmp = a/b;
    if((float)a/b - tmp >= tmp+1 - (float)a/b){
        return a/b+1;
    }
    else{
        return a/b;
